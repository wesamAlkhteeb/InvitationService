...
namespace InvitationCommandService.Application.CommandHandler
{
    public class ChangePermissionsInvitationCommandHandle : IRequestHandler<ChangePermissionInvitationCommand, int>
    {
        ...
        {
            Aggregate aggregate = ChangePermissionsAggragate.GenerateAggregate(new ChangePermissionsInvitationState(), request.subscriptionId, request.memberId, request.Permissions);
